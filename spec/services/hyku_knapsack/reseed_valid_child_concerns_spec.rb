...
RSpec.describe HykuKnapsack::ReseedValidChildConcerns do
  ...
  # Only Portfolio accepts children (issue #169); the item types bar themselves
  # as parents, so their list is emptied.
  let(:item_types) { [PortfolioArtefact, PortfolioEvent, PortfolioLiterature, PortfolioItemCollection] }
  ...
  describe '.call' do
    it 'reflects the fully-registered curation concerns on Portfolio' do
      ...
      expect(Portfolio.valid_child_concerns.map(&:to_s)).to match_array(expected)
      expect(Portfolio.valid_child_concerns.map(&:to_s)).not_to include('GenericWork', 'Image', 'Etd', 'Oer')
    end

    it 'empties the child list for types that bar themselves as parents' do
      item_types.each { |klass| klass.valid_child_concerns = %w[GenericWork Image] }

      described_class.call

      item_types.each { |klass| expect(klass.valid_child_concerns).to be_empty }
    end
